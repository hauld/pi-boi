...
import type { BotContext, BotEvent, BotHandler, ChannelInfo, EventRouter, UserInfo } from "./types.js";

// ============================================================================
// Types
// ============================================================================

export interface SlackEvent extends BotEvent {
	/** Raw Slack file attachments — processed into BotEvent.attachments by logUserMessage */
...
// Re-export generic types for callers that imported them from slack.ts previously
export type { BotContext as SlackContext, BotHandler as MomHandler, ChannelInfo, UserInfo };

// ============================================================================
// Context adapter (Slack-specific BotContext implementation)
// ============================================================================

export function createSlackContext(event: BotEvent, slack: SlackBot, isEvent?: boolean): BotContext {
...
export class SlackBot implements EventRouter {
	...
	/**
	 * Enqueue a synthetic event for processing (used by EventsWatcher).
	 * Always queues (no "already working" rejection).
	 * Returns false if the queue is full (max 5).
	 */
	enqueueEvent(event: BotEvent): boolean {
		...
		const ctx = createSlackContext(event, this, /* isEvent */ true);
		queue.enqueue(() => this.handler.handleEvent(event.channel, ctx, true));
		return true;
	}
		...
		this.socketClient.on("app_mention", ({ event, ack }) => {
			...
			if (slackEvent.text.toLowerCase().trim() === "stop") {
				if (this.handler.isRunning(e.channel)) {
					let stopTs: string | undefined;
					this.handler.handleStop(
						e.channel,
						async () => {
							stopTs = await this.postMessage(e.channel, "_Stopping..._");
						},
						async () => {
							if (stopTs) await this.updateMessage(e.channel, stopTs, "_Stopped_");
							else await this.postMessage(e.channel, "_Stopped_");
						},
			...
			if (this.handler.isRunning(e.channel)) {
				...
				const ctx = createSlackContext(slackEvent, this);
				this.getQueue(e.channel).enqueue(() => this.handler.handleEvent(e.channel, ctx));
			}
